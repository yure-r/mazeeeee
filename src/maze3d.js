...
export default class Maze3D {
  ...
  _addVoidToTemplate() {
      ...
      if (this.constraints.voidSpace[i].length !== 3) {
        throw new TypeError(
          `_addVoidToTemplate Error: Failed to validate voidSpace coordinates because
            ${JSON.stringify(
              this.constraints.voidSpace[i]
            )} at index ${i} has a length of ${
    ...
    for (let coord of this.constraints.voidSpace) {
      if (!this._coordInConstraints(coord) && !this.constraints.sliceOffVoid) {
        ...
        this.mazeTemplate[coord[0]][coord[1]][
          coord[2]
        ] = this.constraints.voidChar;
      }
    }
    ...
    const _cleanBarrierCallback = (cellValue, d, h, w) => {
      ...
      if (cellValue === this.constraints.barrierChar) {
        ...
        for (let key of Object.keys(tempSurrondingValues)) {
          if (
            tempSurrondingValues[key].value === this.constraints.barrierChar
          ) {
            flag = true;
          }
        }
  ...
  solveMaze(start, end) {
    ...
    if (
      ...
      throw new TypeError(
        "solveMaze Error 3: Start and/or End Occupied with void"
      );
    }
    if (
      this._isCharBlockade(this.barrierMaze[start[0]][start[1]][start[2]]) ===
      ...
      throw new TypeError(
        "solveMaze Error 4: Start and/or End Occupied with barrier"
      );
    }
  ...
  _calculateMazeInstanceDetails() {
    ...
    this._mazeInstanceDetails.voidDRC = this.constraints.voidSpace.filter(
      (coord) => this._coordInConstraints(coord)
    );
  ...
  generateModel(options) {
    ...
    if (this._modelOptions.barrier.generate) {
      this._mazeModelMeshComponents.barrierMaterial = new three.MeshStandardMaterial(
        {
          transparent: true,
          wireframe: true,
          visible: this._modelOptions.barrier.visible,
    ...
    if (this._modelOptions.instance) {
      ...
      if (this._modelOptions.barrier.generate) {
        for (let coord of this._mazeInstanceDetails.barrierDRC) {
          ...
          mesh.name =
            "MAZE3D_NO_INSTANCE_MODEL_GROUP_BARRIER_MESH_ITEM" +
            JSON.stringify(coord);
      ...
      if (this._modelOptions.space.generate) {
        for (let coord of this._mazeInstanceDetails.spaceDRC) {
          ...
          mesh.position.set(...cubePosition);
          mesh.name =
            "MAZE3D_NO_INSTANCE_MODEL_GROUP_SPACE_MESH_ITEM_" +
            JSON.stringify(coord);
      ...
      if (this._modelOptions.void.generate) {
        for (let coord of this._mazeInstanceDetails.voidDRC) {
          ...
          mesh.position.set(...cubePosition);
          mesh.name =
            "MAZE3D_NO_INSTANCE_MODEL_GROUP_VOID_MESH_ITEM" +
            JSON.stringify(coord);
      ...
      if (this._modelOptions.map.generate) {
        for (let coord of this._mazeInstanceDetails.mapDRC) {
          ...
          mesh.position.set(...cubePosition);
          mesh.name =
            "MAZE3D_NO_INSTANCE_MODEL_GROUP_MAP_MESH_ITEM" +
            JSON.stringify(coord);
      ...
      if (this._modelOptions.path.generate) {
        for (let coord of this._mazeInstanceDetails.pathDRC) {
          ...
          mesh.position.set(...cubePosition);
          mesh.name =
            "MAZE3D_NO_INSTANCE_MODEL_GROUP_PATH_MESH_ITEM" +
            JSON.stringify(coord);
  ...
  _sliceMaze(groupItem) {
    ...
    if (
      ...
      for (let instanceCoord of this._mazeInstanceDetails[targetDRC]) {
        ...
        for (
          let i = 0;
          i < this._animationSortedInstanceCoords[targetDRC].length;
          i++
        ) {
          // loop through each coord in this bank
          for (let bankCoord of this._animationSortedInstanceCoords[targetDRC][
            i
          ]) {
        ...
        if (foundBank) {
          this._animationSortedInstanceCoords[targetDRC][bankIndex].push([
            ...instanceCoord
          ]);
        } else {
          ...
          for (
            ...
            if (
              this._animationSortedInstanceCoords[targetDRC][i][0][
                targetCoord
              ] > instanceCoord[targetCoord]
            ) {
              this._animationSortedInstanceCoords[targetDRC].splice(i, 0, [
                [...instanceCoord]
              ]);
              wasPushed = true;
              break;
          ...
          if (!wasPushed) {
            this._animationSortedInstanceCoords[targetDRC].push([
              [...instanceCoord]
            ]);
          }
        }
      ...
      for (let coord of this._mappedNumberMazeDRCF) {
        this._sortedMappedNumberMazeDRC[coord[3]].push([
          coord[0],
          coord[1],
          coord[2]
        ]);
  ...
  _animateSlices() {
    ...
    for (let group of groupOrder) {
      ...
      for (let groupItem of group) {
        ...
        for (
          let i = 0;
          i < Object.keys(this._animationInstances[groupItem]).length;
          i++
        ) {
    ...
    for (let group of groupOrder) {
      ...
      for (let groupItem of group) {
        ...
        for (
          ...
          let clipTrackArr = [
            this._generateVisibleKeyTrack(
              timeData,
              currentGroupItemSettings.exit
            )
          ];
  ...
  generateSpotLights(renderer, options) {
    ...
    for (let i = 0; i < lightXYZ.length; i++) {
      ...
      this._sceneLights.shadowHelpers[i] = new three.CameraHelper(
        lights[i].shadow.camera
      );
      // light [x,y,z] = light[x,y,z] + (normal[x,y,z] * mult[x,y,z])
      lights[i].position.x =
        lightXYZ[i][0] + lightNormalXYZ[i][0] * distanceMultXYZ[0];
      lights[i].position.y =
        lightXYZ[i][1] + lightNormalXYZ[i][1] * distanceMultXYZ[1];
      lights[i].position.z =
        lightXYZ[i][2] + lightNormalXYZ[i][2] * distanceMultXYZ[2];
  ...
  modelAPI(action, payload) {
    ...
    if (
      ...
      for (let light of Object.keys(lights)) {
        if (action === "addLights") {
          ...
          this._sceneLights.lightHelpers[i] = new three.SpotLightHelper(
            lights[light],
            0xffffff
          );
      ...
      (action === "addShadowHelpers" ||
        ...
        action === "updateShadowHelpers") &
      this._sceneLights.hasOwnProperty("shadowHelpers")
    ) {
    ...
    for (let key of Object.keys(this.threeModel)) {
      ...
      switch (action) {
        ...
        default:
          throw new TypeError(
            `modelAPI error: action ${action} is not recongized`
          );
      }
    }
    return exitLoad;
        ...
        switch (action) {
          ...
          case "play":
            this._animationSliceMixers[groupItem][slice]
              .clipAction(
                this._animationSliceMixers[groupItem][slice]._actions[0]._clip
              )
              .play();
            break;
          case "update":
            this._animationSliceMixers[groupItem][slice].update(
              payload || 0.016
            );
            break;
          case "removeModel":
            ...
            payload.remove(
              this._animationSliceMixers[groupItem][slice].getRoot()
            );
            this._animationSliceMixers[groupItem][slice]
              .getRoot()
              .material.dispose();
          ...
          default:
            throw new TypeError(
              `Unrecognized animationMixersAPI action ${action}`
            );
        }
      }
    }
